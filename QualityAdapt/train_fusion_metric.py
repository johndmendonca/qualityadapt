"""
Training adapter fusion for response evaluation.
"""
...
class ModelArguments:
    ...
    s_adapter_path: str = field(
        metadata={"help": "Path to Sensibleness adapter"}
    )
    u_adapter_path: str = field(
        metadata={"help": "Path to Understandability adapter"}
    )
...
def main():
    ...
    # Setup adapters
    model.load_adapter(model_args.u_adapter_path,load_as="U", with_head=False)
    model.load_adapter(model_args.s_adapter_path,load_as="S", with_head=False)
    
    adapter_setup = Fuse("U", "S")
    ...
    def preprocess_function(examples):
        # Tokenize the texts
        sentence1_key, sentence2_key = 'ctx', 'res'
        args = (
            (examples[sentence1_key],) if sentence2_key is None else (examples[sentence1_key], examples[sentence2_key])
        )
        result = tokenizer(*args, padding=padding, max_length=max_length, truncation=True)
        result["labels"] = (np.array(examples["label"])-1)/4

        return result

    def preprocess_function_human(examples):
        # Tokenize the texts
        sentence1_key, sentence2_key = 'ctx', 'model_response'
        args = (
            (examples[sentence1_key],) if sentence2_key is None else (examples[sentence1_key], examples[sentence2_key])
        )
        result = tokenizer(*args, padding=padding, max_length=max_length, truncation=True)
        # Get average Sensibleness
        result["H"] = np.mean([examples["EO1"], examples["EO2"], examples["EO3"],examples["EO4"]],axis=0)
        result["labels"] = (np.array(result["H"])-1)/3
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        return {"pearson_r": stats.pearsonr(preds,p.label_ids), "spearman_r" : (rho,p_value)}
    ...
    if training_args.do_predict:
        ...
        for test_dataset, task in zip(test_datasets, tasks):
            ...
            output_test_file = os.path.join(training_args.output_dir, f"test_results_{task}_{i}.txt")
            if trainer.is_world_process_zero():
